...
async def _contact(db, name, channel, external_id, *, contacted_days_ago=None):
    doc = await contacts_engine.resolve_or_create(
        db, "plucia", channel, external_id, name=name
    )
...
async def test_exact_name_beats_substring(db, no_directory):
    await _contact(db, "Ari", "email", "ari@plucia.com", contacted_days_ago=90)
    await _contact(db, "Ariyaman Dev", "email", "ariyaman@plucia.com", contacted_days_ago=1)

    found = await recipients.find_recipients(db, "plucia", "u1", "Ari")
...
async def test_recency_breaks_ties(db, no_directory):
    await _contact(db, "Ariyaman Old", "email", "old@plucia.com", contacted_days_ago=100)
    await _contact(db, "Ariyaman New", "email", "new@plucia.com", contacted_days_ago=2)
...
async def test_matches_a_middle_word(db, no_directory):
    ...
    await _contact(db, "Dev Ariyaman", "email", "dev@plucia.com")
    found = await recipients.find_recipients(db, "plucia", "u1", "ariyaman")
...
async def test_address_query_finds_the_contact_regardless_of_formatting(db, no_directory):
    ...
    found = await recipients.find_recipients(db, "plucia", "u1", "ADA@Example.com")
    assert len(found) == 1
...
async def test_no_match_returns_empty(db, no_directory):
    await _contact(db, "Ada", "email", "ada@example.com")
    assert await recipients.find_recipients(db, "plucia", "u1", "Nobody") == []
...
async def test_finds_someone_only_in_the_mailbox(db, monkeypatch):
    ...
    assert found[0]["channels"] == [{"channel": "email", "id": "ariyaman@plucia.com"}]
...
async def test_crm_and_mailbox_dedupe_on_address(db, monkeypatch):
    ...
    found = await recipients.find_recipients(db, "plucia", "u1", "Ada")
...
async def test_mailbox_is_skipped_without_a_user(db, monkeypatch):
    ...
    assert await recipients.find_recipients(db, "plucia", None, "X") == []
...
async def test_raw_address_resolves_to_itself(db, no_directory):
    ...
    external_id, name = await recipients.resolve_destination(
        db, "plucia", "u1", "Ariyaman@Plucia.com", "email"
    )
    assert external_id == "ariyaman@plucia.com"
    assert name is None
...
async def test_unresolvable_name_returns_nothing(db, no_directory):
    assert await recipients.resolve_destination(
        db, "plucia", "u1", "Nobody At All", "email"
    ) == (None, None)
